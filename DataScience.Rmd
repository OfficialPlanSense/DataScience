...
# View first few rows
#head(stndata$NORTHGLENN) # date field
#head(stndata)
...
# View first few rows
#head(data$first_occurrence_date) # date field
#head(data)
...
combined_data <- merge(accidents_per_day, rainfall_data,  
                       by.x = "first_occurrence_date", by.y = "NORTHGLENN_date", all = TRUE)  # use 'all = TRUE' to keep all rows

# Step 4: Convert dates to numeric values (days since 1970-01-01)
combined_data$first_occurrence_numeric <- as.numeric(combined_data$first_occurrence_date)

# Step 5: Ensure 'rain_value' is numeric, and handle NAs by replacing them with 0
...
# Step 6: Perform Pearson's correlation test on numeric values
...
# Adjust accident count aggregation
...
plot <- ggplot(combined_data, aes(x = first_occurrence_date)) +
  ...
  scale_color_manual(values = c("Accidents" = "#E63946")) +  # Smooth red line for accidents
  scale_fill_manual(values = c("Rainfall" = "#457B9D")) +  # Soft blue for rainfall bars
  theme_classic(base_size = 14) +  # Clean theme with readable text
